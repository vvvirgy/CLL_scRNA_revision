...
lapply(genes_list %>% names, function(g) {
  DotPlot(object = objs_by_time$s19$T3, features = c(genes_list[[g]], 'CXCR4', 'CD27', 'CD5'), cols = 'RdBu',
  ...
  # ggsave(paste0('other_projects/', g, '_different_populations_s19_t3.png'), width = 13, height = 5)
  ggsave(paste0('other_projects/', g, '_different_populations_s19_t3.pdf'), width = 9, height = 4)
...
lapply(genes_list %>% names, function(g) {
  DotPlot(object = objs_by_time$s3299$T1, features = c(genes_list[[g]], 'CXCR4', 'CD27', 'CD5'), cols = 'RdBu',
          idents = c('CXCR4hiCD27lo', 'CXCR4loCD27hi'), dot.scale = 19) +
    theme(axis.text.x = element_text(angle = 35, vjust = 1, hjust = 1, size = 16), 
          legend.position = 'right', legend.direction = 'vertical', 
          axis.line = element_line(linewidth = 0.3), 
  ...
  # ggsave(paste0('other_projects/', g, '_different_populations_s19_t3.png'), width = 13, height = 5)
  ggsave(paste0('other_projects/', g, '_different_populations_s3299_t1.pdf'), width = 9, height = 4)
...
dge_v2 = readRDS('Hemasphere_data/dge_CXCR4hiCD27lo_vs_CXCR4loCD27hi_s19_s3299.rds')

genes = unlist(genes_list)

res_dge_all = lapply(dge_v2 %>% names, function(x) {
  lapply(dge_v2[[x]] %>% names, function(s) {
    dge_v2[[x]][[s]] %>% 
      tibble::rownames_to_column('gene') %>% 
      mutate(sample = paste(x, s, sep = '_')) %>% 
      filter(gene %in% c(genes, 'CXCR4', 'CD27', 'CD5'))
  }) %>% bind_rows()
}) %>% bind_rows() 

nadeu_dge = res_dge_all %>% 
  dplyr::filter(sample %in% c('s19_T3', 's3299_T1'))

write.table(nadeu_dge, 'other_projects/dge_nadeu.csv', sep = ',', quote = F, row.names = F, col.names = T)
